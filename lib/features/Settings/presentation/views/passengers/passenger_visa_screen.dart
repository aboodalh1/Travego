...
class PassengerVisaScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => PassengerCubit(getIt.get<PassengersRepoImpl>())..getPassengerVisa(token: token, id: id),
        ...
        builder: (context, state) {
          ...
          if(state is PassengerAddInfoError){
            return Column(
              ...
              children: [
                AlertDialog(
                  title: Text(state.error =='404' ? '$name didn\'t add visa yet' : 'something went wrong'),
                  content: Center(child: DefaultElevated(label: 'add visa', fill: true, onPressed:(){
                    showDialog(context: context, builder: (context)=>EditVisaWidget(id:id,name: name, passengerCubit: passengerCubit));
                  }),),
                ),
              ],
            );
          }
          return EditVisaWidget(name: name, passengerCubit: passengerCubit,id:id);
        },
      ),
    );
  }
            ...
            child: Column(
              ...
              children: [
                ...
                DefaultElevated(label: 'save', fill: true, onPressed:(){
                  passengerCubit.addPassengerVisa(country: passengerCubit.visaCountry.text,
